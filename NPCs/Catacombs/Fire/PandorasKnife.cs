	...
	public class PandorasKnife : ModNPC
	{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			...
			for (float i = 0f; i < 1f; i += 0.25f)
			{
				...
				spriteBatch.Draw(texture, drawPos + new Vector2(0f, 8f).RotatedBy(radians) * time, NPC.frame, new Color(255, 70, 90, 50), NPC.rotation, frameOrigin, NPC.scale, SpriteEffects.None, 0);
			}
			...
			for (float i = 0f; i < 1f; i += 0.34f)
			{
				...
				spriteBatch.Draw(texture, drawPos + new Vector2(0f, 4f).RotatedBy(radians) * time, NPC.frame, new Color(255, 120, 60, 77), NPC.rotation, frameOrigin, NPC.scale, SpriteEffects.None, 0);
			}
