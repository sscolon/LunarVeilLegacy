...
namespace LunarVeilLegacy.NPCs.Bosses.GothiviaTheSun.GOS.Projectiles
{
    public class Sun2 : ModNPC
	{
		...
		public override void HitEffect(NPC.HitInfo hit)
		{
			for (int k = 0; k < 11; k++)
			{
				Dust.NewDust(NPC.position, NPC.width, NPC.height, DustID.CoralTorch, 1, -1f, 1, default, .61f);
			}
		}
        ...
        public override void AI()
		{
			...
			if (gr == 120 && !HHH)
			{
                ...
                if (StellaMultiplayer.IsHost)
                {
                    int type = ModContent.ProjectileType<SunLaserProj2>();
                    int damage = 800;
                    int knockback = 1;
