...
namespace LunarVeilLegacy.NPCs.Town
{
	...
	public class Ishtar : ModNPC
	{
		...
		public class IshtarPersonProfile : ITownNPCProfile
		{
			...
			public Asset<Texture2D> GetTextureNPCShouldUse(NPC npc)
			{
				if (npc.IsABestiaryIconDummy && !npc.ForcePartyHatOn)
					return ModContent.Request<Texture2D>("LunarVeilLegacy/NPCs/Town/Ishtar");

				if (npc.altTexture == 1)
					return ModContent.Request<Texture2D>("LunarVeilLegacy/NPCs/Town/Ishtar_Head");

				return ModContent.Request<Texture2D>("LunarVeilLegacy/NPCs/Town/Ishtar");
			}

			public int GetHeadTextureIndex(NPC npc) => ModContent.GetModHeadSlot("LunarVeilLegacy/NPCs/Town/Ishtar_Head");
		}
		...
		public override void HitEffect(NPC.HitInfo hit)
		{
			...
			for (int k = 0; k < num; k++)
			{
				Dust.NewDust(NPC.position, NPC.width, NPC.height, DustID.BoneTorch);
			}
		}
