    ...
    internal class DreadmireBag : ModItem
    {
        ...
        public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
        {
            ...
            for (float i = 0f; i < 1f; i += 0.25f)
            {
                ...
                spriteBatch.Draw(texture, drawPos + new Vector2(0f, 8f).RotatedBy(radians) * time, frame, new Color(99, 39, 51, 50), rotation, frameOrigin, scale, SpriteEffects.None, 0);
            }
            ...
            for (float i = 0f; i < 1f; i += 0.34f)
            {
                ...
                spriteBatch.Draw(texture, drawPos + new Vector2(0f, 4f).RotatedBy(radians) * time, frame, new Color(255, 8, 55, 77), rotation, frameOrigin, scale, SpriteEffects.None, 0);
            }
