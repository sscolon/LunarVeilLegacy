...
{
    public class GutinierSlashParticle : Particle
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color lightColor)
        {
            ...
            Texture2D tex3 = Request<Texture2D>("LunarVeilLegacy/Particles/AuroranSlash").Value;
            Texture2D tex4 = Request<Texture2D>("LunarVeilLegacy/Particles/AuroranSlashOpp").Value;

            float alpha = timeLeft <= 20 ? 1f - 1f / 20f * (20 - timeLeft) : 1f;
            if (alpha < 0f) alpha = 0f;
            Color color = Color.Multiply(new(1.2f, 1.2f, 1.2f, 0), alpha);
