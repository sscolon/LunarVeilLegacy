...
{
    public class SwingaSlash : Particle
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color lightColor)
        {
            ...
            Texture2D tex3 = Request<Texture2D>("LunarVeilLegacy/Particles/SwingaSlash").Value;
            Texture2D tex4 = Request<Texture2D>("LunarVeilLegacy/Particles/SwingaSlashOpp").Value;

            float alpha = timeLeft <= 20 ? 1f - 1f / 20f * (20 - timeLeft) : 1f;
            if (alpha < 0f) alpha = 0f;
            Color color = Color.Multiply(new(1.5f, 1.5f, 1.5f, 0), alpha);
            ...
            {
                spriteBatch.Draw(tex4, Center - Main.screenPosition, tex4.AnimationFrame(ref frameCount, ref frameTick, 1, 16, true), color, velocity.ToRotation() + 180, new Vector2(150f, 150f) * 0.5f, 3f * Scale, spriteEffects, 0f);


            }
            else
            {
                spriteBatch.Draw(tex3, Center - Main.screenPosition, tex3.AnimationFrame(ref frameCount, ref frameTick, 1, 16, true), color, velocity.ToRotation() + 180, new Vector2(150f, 150f) * 0.5f, 3f * Scale, spriteEffects, 0f);
