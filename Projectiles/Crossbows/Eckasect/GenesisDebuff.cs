...
{
	public class GenesisDebuff : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (greenber == 60)
			{
				...
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X + speedXa, Projectile.position.Y + speedYa, speedXa * 0, speedYa * 0, ModContent.ProjectileType<GenesisBubble>(), (int)(Projectile.damage * 0), 0f, Projectile.owner, 0f, 0f);
				greenber = 0;
			}
		...
		public NPC FindClosestNPCS(float maxDetectDistance)
		{
			...
			for (int k = 0; k < Main.maxNPCs; k++)
			{
				...
				if (target.CanBeChasedBy() && target.HasBuff<Genesis>())
				{
