...
{
    public class TwilitTomeRed : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int k = 0; k < Projectile.oldPos.Length; k++)
            {
                ...
                Color color = Projectile.GetAlpha(Color.Lerp(new Color(236, 118, 126), new Color(226, 46, 57), 1f / Projectile.oldPos.Length * k) * (1f - 1f / Projectile.oldPos.Length * k));
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 180; i++)
            {
                int num = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.RedTorch, 0f, -2f, 0, default, 1.5f);
            ...
            for (int i = 0; i < 80; i++)
            {
                int num = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.RedTorch, 0f, -2f, 0, default, 2.5f);
                Main.dust[num].noGravity = true;
                Main.dust[num].position.X += Main.rand.Next(-50, 51) * .05f - 1.5f;
                Main.dust[num].position.X += Main.rand.Next(-50, 51) * .05f - 1.5f;
                {
