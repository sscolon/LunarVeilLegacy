...
namespace LunarVeilLegacy.Projectiles.Bow
{
    internal class CrysalizerArrow3 : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustID.BlueFairy, (Vector2.One * Main.rand.Next(1, 5)).RotatedByRandom(25.0), 0, default, 1f).noGravity = false;
            }
            for (int i = 0; i < 50; i++)
            {
                int num = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.BlueFairy, 0f, -2f, 0, default(Color), 1.5f);
        ...
        {
            return Color.Lerp(Color.CornflowerBlue, Color.Transparent, completionRatio) * 0.7f;
        }
