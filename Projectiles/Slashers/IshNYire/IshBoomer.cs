...
{
	public class IshBoomer : ModProjectile
	{
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			...
			if (!bounced)
			{
				...
				for (int i = 0; i < 10; i++)
				{
					Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<GlowDust>(), (Vector2.One * Main.rand.Next(1, 5)).RotatedByRandom(19.0), 0, Color.OrangeRed, 0.7f).noGravity = true;
				}
				for (int i = 0; i < 6; i++)
				{
				...
				switch (Main.rand.Next(2))
				{
					...
					//	Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.position.X, player.position.Y, 0, 0, ModContent.ProjectileType<Hulthit1>(), (int)(Projectile.damage * 1.5), 0f, Projectile.owner, 0f, 0f);
						...
						for (int i = 0; i < 26; i++)
						{
							Dust.NewDustPerfect(target.Center, ModContent.DustType<GlowDust>(), (Vector2.One * Main.rand.Next(1, 9)).RotatedByRandom(MathHelper.TwoPi), 0, Color.LightGoldenrodYellow, 1f).noGravity = true;
						}
						for (int i = 0; i < 20; i++)
						{
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			var TrailTex2 = ModContent.Request<Texture2D>("LunarVeilLegacy/Effects/Primitives/Trails/MetalTrail").Value;
			...
			if (SwordSlash == null)
			{
				SwordSlash = new TrailRenderer(TrailTex, TrailRenderer.DefaultPass, (p) => new Vector2(80f), (p) => new Color(250, 110, 15, 90) * (1f - p));
				SwordSlash.drawOffset = Projectile.Size / 1.8f;
			}
			if (SwordSlash2 == null)
			{
				SwordSlash2 = new TrailRenderer(TrailTex2, TrailRenderer.DefaultPass, (p) => new Vector2(100f), (p) => new Color(200, 15, 25, 100) * (1f - p));
				SwordSlash2.drawOffset = Projectile.Size / 1.9f;

			}
			if (SwordSlash3 == null)
			{
				SwordSlash3 = new TrailRenderer(TrailTex3, TrailRenderer.DefaultPass, (p) => new Vector2(130f), (p) => new Color(250, 25, 25, 50) * (1f - p));
			...
			if (SwordSlash4 == null)
			{
				SwordSlash4 = new TrailRenderer(TrailTex3, TrailRenderer.DefaultPass, (p) => new Vector2(70f), (p) => new Color(255, 52, 25, 50) * (1f - p));
