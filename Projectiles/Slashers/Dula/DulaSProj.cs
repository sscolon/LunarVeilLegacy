...
{
    public class DulaSProj : ModProjectile
    {
        ...
        {
            if (Projectile.timeLeft % (SwingTime / 8) == 0)
            {
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Owner.Center, Vector2.Zero,
                    ModContent.ProjectileType<DulaSwirl>(), Projectile.damage / 6, 1, Owner.whoAmI);
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (SwordSlash == null)
            {
                SwordSlash = new TrailRenderer(TrailTex, TrailRenderer.DefaultPass, (p) => new Vector2(100f), (p) => new Color(200, 150, 25, 90) * (1f - p));
                SwordSlash.drawOffset = Projectile.Size / 1.8f;
            }
            if (SwordSlash2 == null)
            {
                SwordSlash2 = new TrailRenderer(TrailTex2, TrailRenderer.DefaultPass, (p) => new Vector2(150f), (p) => new Color(250, 150, 25, 100) * (1f - p));
                SwordSlash2.drawOffset = Projectile.Size / 1.9f;

            }
            if (SwordSlash3 == null)
            {
                SwordSlash3 = new TrailRenderer(TrailTex3, TrailRenderer.DefaultPass, (p) => new Vector2(180f), (p) => new Color(255, 105, 25, 90) * (1f - p));
            ...
            if (SwordSlash4 == null)
            {
                SwordSlash4 = new TrailRenderer(TrailTex3, TrailRenderer.DefaultPass, (p) => new Vector2(150f), (p) => new Color(255, 255, 255, 110) * (1f - p));
