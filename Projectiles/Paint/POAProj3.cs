...
{
    public class POAProj3 : ModProjectile, IPixelPrimitiveDrawer
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (Main.rand.NextBool(4))
            {
                float speedXa = Main.rand.NextFloat(-35f, 35f);
                float speedYa = Main.rand.Next(-35, 35);
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X + speedXa, Projectile.Center.Y + speedYa, 0, 0, ModContent.ProjectileType<PaintBomb3>(), (Projectile.damage * 2) + player.GetModPlayer<MyPlayer>().PPPaintDMG2, 1, Projectile.owner, 0, 0);
