...
namespace LunarVeilLegacy.Projectiles.Steins
{
	public class IkFist : ModProjectile
	{
			...
			SoundEngine.PlaySound(new SoundStyle("LunarVeilLegacy/Assets/Sounds/SteinIk") { Pitch = Main.rand.NextFloat(-0.5f, 0.5f) });
				...
				switch (Main.rand.Next(7))
				{
					...
					case 2:
						...
						break;
					case 3:
						Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.position.X, player.position.Y, 0, 0, ModContent.ProjectileType<SEXY>(), (int)(Projectile.damage * 1), 0f, Projectile.owner, 0f, 0f);
						break;
					case 4:
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			if (SwordSlash == null)
			{
				SwordSlash = new TrailRenderer(TrailTex, TrailRenderer.DefaultPass, (p) => new Vector2(25f), (p) => new Color(250, 15, 0, 90) * (1f - p));
				SwordSlash.drawOffset = Projectile.Size / 1.8f;
			}
			if (SwordSlash2 == null)
			{
				SwordSlash2 = new TrailRenderer(TrailTex2, TrailRenderer.DefaultPass, (p) => new Vector2(40f), (p) => new Color(10, 15, 250, 100) * (1f - p));
				SwordSlash2.drawOffset = Projectile.Size / 1.9f;

			}
			if (SwordSlash3 == null)
			{
				SwordSlash3 = new TrailRenderer(TrailTex3, TrailRenderer.DefaultPass, (p) => new Vector2(50f), (p) => new Color(250, 0, 25, 90) * (1f - p));
			...
			if (SwordSlash4 == null)
			{
				SwordSlash4 = new TrailRenderer(TrailTex3, TrailRenderer.DefaultPass, (p) => new Vector2(40f), (p) => new Color(2, 2, 255, 110) * (1f - p));
