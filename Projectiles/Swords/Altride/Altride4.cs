...
namespace LunarVeilLegacy.Projectiles.Swords.Altride
{
    internal class Altride4 : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (Sound == 1)
            {
                SoundEngine.PlaySound(new SoundStyle("LunarVeilLegacy/Assets/Sounds/Crysalizer4"), Projectile.position);
            }
            else
            {
                SoundEngine.PlaySound(new SoundStyle("LunarVeilLegacy/Assets/Sounds/Crysalizer5"), Projectile.position);
            }

            for (int i = 0; i < 14; i++)
            {
                ParticleManager.NewParticle(Projectile.Center, (Vector2.One * Main.rand.Next(1, 12)).RotatedByRandom(10.0), ParticleManager.NewInstance<morrowstar>(), Color.RoyalBlue, Main.rand.NextFloat(0.2f, 0.8f));
            }
