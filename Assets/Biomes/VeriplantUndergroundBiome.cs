...
{
    public class VeriplantUndergroundBiome : ModBiome
	{
		...
		public override SceneEffectPriority Priority => SceneEffectPriority.BiomeMedium; // We have set the SceneEffectPriority to be BiomeLow for purpose of example, however default behavour is BiomeLow.
		...
		public override bool IsBiomeActive(Player player) => BiomeTileCounts.InVeri;
		public override void OnEnter(Player player) => player.GetModPlayer<MyPlayer>().ZoneVeri = true;
		public override void OnLeave(Player player) => player.GetModPlayer<MyPlayer>().ZoneVeri= false;
